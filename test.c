...
void PortConfiguration(void)
{
    ...
    /*
        output 인 것들 Hz 어떻게 해야할지 모르겠어서 그냥 다 11로 한거임!!
    */
    /* MCO Pin Configuration */ // 8번 핀
    GPIOA->CRH |= (uint32_t)(GPIO_CRH_CNF8_1 | GPIO_CRH_MODE8);
    /* USART Pin Configuration */
    GPIOA->CRH |= (uint32_t)(GPIO_CRH_CNF9_1 | GPIO_CRH_MODE9 |
                             GPIO_CRH_CNF10_1);

    /* Reset(Clear) Port D CRH - User S1 Button */ // 11번 핀 input!!!
...
void UartInit(void)
{
    ...
    // 0: 1 Start bit, 8 Data bits, n Stop bit (27.6.4 bit 12 M:Word length)
    // 초기화 할 때 설정되었음!

    /* Set PCE and PS bits according to USART_Parity value */
    //@TODO - 7: Parity : None
    // 0: Parity control disabled (27.6.4 bit 10 PCE : Parity control enable)
    // 0: Even parity (27.6.4 bit 9 PS : Parity selection)
    // 초기화 할 때 설정되었음!
    ...
    // 00: 1 Stop bit (27.6.5 bits 13:12 STOP : STOP bits)
    // 초기화 할 때 설정되었음!
    ...
    // 0: CTS hardware flow control disabled (27.6.6 bit 9 CTSE:CTSenable)
    // 0: RTS hardware flow control disabled (27.6.6 bit 8 RTSE:RTSenable)
    // 초기화 할 때 설정되었음!
